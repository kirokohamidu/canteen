...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_SESSION['role'] === 'canteen_manager') {
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'create' && $name && $price > 0) {
        $stmt = $dsn->prepare('INSERT INTO menu_items (name, description, price) VALUES (?, ?, ?)');
    ...
    if ($action === 'update') {
        ...
        if ($id && $name && $price > 0) {
            $stmt = $dsn->prepare('UPDATE menu_items SET name = ?, description = ?, price = ? WHERE id = ?');
    ...
    if ($action === 'delete') {
        $id = intval($_POST['id'] ?? 0);
        if ($id) {
            $stmt = $dsn->prepare('DELETE FROM menu_items WHERE id = ?');
            $stmt->execute([$id]);
            redirect('menu.php');
        }
    }
...
$editItem = null;
if (isset($_GET['edit']) && $_SESSION['role'] === 'canteen_manager') {
    $id = intval($_GET['edit']);
...
<body class="menu-page" style="background: linear-gradient(rgba(15, 23, 42, 0.35), rgba(15, 23, 42, 0.35)), url('https://elearning.umu.ac.ug/pluginfile.php?file=%2F1%2Ftheme_academi%2Fslide2image%2F1776174832%2FUganda%20Martyrs%20University%20%281%29%281%29%20%281%29.webp');">
    ...
    <div class="container">
        ...
        <?php if ($_SESSION['role'] === 'canteen_manager'): ?>
